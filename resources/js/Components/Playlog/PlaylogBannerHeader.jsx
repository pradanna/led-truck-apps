...
export default function PlaylogBannerHeader({ onAddMaterial, selectedTruck = 'truck_1' }) {
    const { auth } = usePage().props;
    const isAdmin = auth?.user?.isAdmin ?? (auth?.user?.role === 'admin');

    const handleSwitchTruck = (truckId) => {
        router.get('/playlog', { truck_id: truckId }, { preserveState: false, preserveScroll: true });
    };
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
                            Sistem Playlog & Novastar Videotron {selectedTruck === 'truck_2' ? '- Truk LED 02' : '- Truk LED 01'}
                        </h2>
